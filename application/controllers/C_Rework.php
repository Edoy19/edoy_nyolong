...
class C_Rework extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->database('db_mfg_portal');
		$this->load->database('db_central_user');
		//model tambahan
		// $this->load->model('ProdakModel');
		// $this->load->model('RoleModel');
		// $this->load->model('MfgModel');
	...
	public function Index()
	{
		...
		$data['change_sortir'] = $this->M_Rework->get_tb_change_sortir(); // Menarik dan menampung ke data nik
		$data['qc'] = $this->M_Rework->get_tb_qc3(); // Menarik dan menampung ke data nik
		$data['spv'] = $this->M_Rework->get_tb_spv2(); // Menarik dan menampung ke data nik
		...
		// $data['spv_id'] = $this->ProdakModel->get_supervisor_id();
		...
		$this->load->view('rework/layout/header'); //Tampil header
		...
		$this->load->view('rework/V_Rework_copy', $data); // Tampil data
	...
	public function ajax_add()
	{
		...
		// ********************* 5. Simpan data     *********************
		$where = array('id_request' => $this->input->post('nik'));

		$this->M_Rework->Input_Data($post_datamerge, 'rework');
		$this->M_Rework->Update_Data_Sortir($where, $post_update, 'sortir');
		$this->M_Rework->Update_Data_Perubahan($where, $post_update, 'change_sortir');
		$this->M_Rework->Update_Data_Result($where, $post_update, 'tb_result');
	...
	{
		// ********************* 0. Generate nomor transaksi  *********************          
		$mdate = "APR" . mdate('%Y%m', time());
		$idapprove = $this->M_Rework->Max_data_approve($mdate, 'approval')->row();
		if ($idapprove->id == NULL) {
			// Jika belum ada
			$id4 = $mdate . "001";
		} else {
			// Jika sudah ada maka naik 1 level
			$id4 = $idapprove->id; // Jika sudah ada 
			$id4 = "APR" . (intval(substr($id4, 3, 11)) + 1);
			// $number = intval(substr($id2->id,3,11)) + 1; // Ambil bagian numerik dan tambahkan 1
			// $id4 = $mdate . str_pad($number, 1, STR_PAD_LEFT); // Tambahkan padding nol jika diperlukan
		}
		$id = $id4;
		// ********************* 1. Set id  *********************
		$post_data2 = array('id_approval' => $id4);
		// ********************* 2. Transaction date  *********************
		$post_data3 = array('transaction_date' => mdate('%Y-%m-%d', time()));
		// ******************** 3. Collect all data post *********************     
		$post_data = $this->input->post();
		...
		// ********************* 4. Merge data post *********************        
		$post_datamerge = array_merge($post_data, $post_data2, $post_data3, $tgl_approval);
		// ********************* 5. Simpan data     *********************
		$this->M_Rework->Input_Data($post_datamerge, 'approval');
		// ********************* 6. Upload file jika ada  *********************   
		$data['status'] = $msg;
		// return value array
		$this->output->set_content_type('application/json')->set_output(json_encode($data));
	...
	public function ajax_delete()
	{
		...
		$iswhere = array('id_request' => $this->input->post('hdrid'));
		$nowhere = array('id_sortir' => $id_sortir);
		$this->M_Rework->Delete_Data($where, 'rework');
		$this->M_Rework->Delete_Data($iswhere, 'sortir');
		$this->M_Rework->Delete_Data($iswhere, 'change_sortir');
		$this->M_Rework->Delete_Data($nowhere, 'tb_result');
