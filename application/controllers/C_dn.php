...
class C_dn extends CI_Controller {
	...
	public function Index()
	{
        ...
        $data['account'] = $this->M_dn->Tampil_account()->row();// Menarik dan menampung ke data account
        $data['transfer_to'] = $this->M_dn->Tampil_transfer_to()->row();// Menarik dan menampung ke data transfer to
        ...
        $data['director_filter'] =$this->M_dn->Tampil_director_filter();
    ...
    function view_data_where()
    {
        ...
        if ($menu_name=='Add Debit Note') { // Kondisi jika menu_name = Add settlement
            ...
            if($this->session->userdata('rolename')=='Administrator FA' || $this->session->userdata('rolename')=='Super Admin'  ){
                $tables = "fn_view_sum_dn('')"; // Terbuka semua tanpa user
            }else{
                    $tables = "fn_view_sum_dn('$nik')"; // Maka table yang dipakai
            }
        ...
        // $search = array('hdrid','date_of_issue','name','nik','status_transaction','section','company_to','company_address','attention','attention_departement','detail_request','reference','budget','account','payment_terms','banyak_cicilan','transfer_to','transfer_address','currency','idr','usd','eur','swift_code','account_name','amount','amount_total','amount_in_word','dpp','vat','vat_persen','vat_nominal','vat_total','dmia_bank_account','transfer_name','bank_name','account_number','attachment1','attachment2','attachment3','attachment4','director','title','approver_dept_user','approver_dept_finance','finish_settlement_dn_bank','finish_settlement_dn_amount','finish_settlement_dn_date');//isi table dn
        $search = array('hdrid','date_of_issue','name','nik','status_transaction','section','company_to','company_address','attention','attention_departement','detail_request','reference','budget','account','payment_terms','banyak_cicilan','transfer_to','transfer_address','currency','idr','usd','eur','swift_code','account_name','amount','amount_total','amount_in_word','dpp','vat','vat_persen','vat_nominal','vat_total','dmia_bank_account','transfer_name','bank_name','account_number','attachment1','attachment2','attachment3','attachment4','director','title','approver_dept_user','approver_dept_finance','finish_settlement_dn_bank','finish_settlement_dn_amount','finish_settlement_dn_date','start_month','company_address1','company_address2','email_and_telp');//isi table cn
    ...
    function View_cicilan_dn()
    {
        
        $tables = "tb_cicilan_dn"; // Table yang digunakan     
    ...
    public function ajax_add()
	{
        ...
        if ($hdrid2->hdrid==NULL){ // Jika hdrid kosong
           ...
           $hdrid3=$mdate."001";// request user star dari no 037
        }else{
           ...
           $nomor=str_pad($nomor,3,"0",STR_PAD_LEFT); //untuk mengambil huruf sesuai diatur
        ...
        $this->M_dn->Update_Data($where,$post_datamerge,'tb_cicilan_dn');// Buat kondisi where untuk dikirim ke model
    ...
    public function ajax_update_dn()
	{
    ...
    /// @see ajax_update_date()
    /// @note Update date of issue
    /// @attention Update date berdasarkan hdrid(Adminstrator FA OR Super Admin ONLY)
    public function ajax_update_date()
	{

         // ********************* 1. Collect data post *********************
        $date_of_issue=$this->input->post('date_of_issue'); // Tarik data input post date_of_issue
        $hdrid=$this->input->post('hdrid'); // Tarik data input post hdrid
        $post_data = array('date_of_issue'=>$date_of_issue); // Buat kondisi field,data yang mau di update
        $where = array('hdrid' => $hdrid);// Buat kondisi where untuk dikirim ke model   
        $post_datamerge=array_merge($post_data,$post_data);//menggabungkan semua data
    ...
    function ajax_getbytrxid(){      
        ...
        $data=$this->M_dn->ajax_getbytrxid($hdrid,$no_cicilan,'tb_cicilan_dn')->row();// Mengirim param ke model untuk query 
        echo json_encode($data);// mengembalikkan nilai dari format daTA json
    ...
    function Ajax_Add_Row(){      
        ...
        $hdrid2=$this->M_dn->Max_data_dn($hdridm,'tb_cicilan_dn')->row();   // Mengambil row dari database
        ...
        $this->M_dn->Input_Data($post_datamerge,'tb_cicilan_dn'); // Mengirim parameter ke model untuk query 
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            $config['max_size']  = '2000000'; // Configurasi max size dari file yang di upload
