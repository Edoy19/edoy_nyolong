...
class M_customer_name extends CI_Model {
   ...
   {
      $this->db->select_max('custno');     
      $this->db->like('custno',$mdate);
   ...
   /// @see ajax_getbycustno()
   /// @note
   /// @attention
   function ajax_getbycustno($custno,$table){      
      return $this->db->get_where($table, array('custno' => $custno));
   }
   ...
   /// @note
   /// @attention
   public function Send_mail($custno)
   {
    ...
    $query =$this->db->query("
     ...
     where custno='WTR2010009' and level_approve=(select min(level_approve) from customer_name_approver where custno='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       $where = array('custno' => $query->custno); 
      ...
      $this->db->query("CALL sp_send_mail @custno='$query->custno',@nik='$query->nik_approver',@name='$query->name_approver',@email='widodo.a5v@ap.denso.com',@EmailSubject='Waiting Approver Wire transfer No ',@Email_body_content='You need approver'");
     ...
     $this->db->query("exec sp_send_mail @custno='123',@nik='DM1902060',@name='Widodo',@email='widodo.a5v@ap.denso.com',@EmailSubject='Finish Approver Wire transfer No ',@Email_body_content='You need approver'");
   ...
   /// @note
   /// @attention
   public function Send_mail_reject($custno)
   {
    
      // Cari data wire
      $query =$this->db->query("select custno,nik,name_requester,isnull(reject_by,'')reject_by,isnull(reason_reject,'')reason_reject from tb_wiretransfer where custno='$custno' ");
      $query=$query->row();
      ...
      if ($query2->num_rows()>0) {
          $query2=$query2->row();
         //  Kirim email
          $result =$this->db->query("exec sp_send_mail @custno='$query->custno',@nik='$query->nik',@name='$query->name_requester',@email='widodo.a5v@ap.denso.com',@EmailSubject='Rejected Wire transfer No ',@Email_body_content='Rejected by <b> $query->reject_by </b>',@comment='With reason = <b> $query->reason_reject </b>'")->result_array();
      }
