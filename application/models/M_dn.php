...
class M_dn extends CI_Model
{
	...
	/// @note Select data hdrid terbesar
	/// @attention Untuk membuat hdrid di ajax_add
	public function Max_data_dn($hdridm, $table)
	{
		$this->db->select_max('no_cicilan');
	...
	/// @note Select data
	/// @attention select data berdasarkan hdrid 
	function ajax_getbytrxid($hdrid, $no_cicilan, $table)
	{
		return $this->db->get_where($table, array('hdrid' => $hdrid, 'no_cicilan' => $no_cicilan));
	}
	...
	{
		$DB3 = $this->load->database('db_dncn', TRUE);
		$query = $DB3->get('tb_company_to')->result();
		$DB3->Close();
		return  $query;
	}
	...
	{
		$DB4 = $this->load->database('db_dncn', TRUE);
		$query = $DB4->get('tb_akun_dn')->result();
		$DB4->Close();
		return  $query;
	}
	...
	{
		$this->db->select('*');
		$this->db->where('hdrid', 'TR202211001');
		$this->db->from('tb_director');
		return $this->db->get()->row();
	}

	/// @see Tampil_director_filter()
	/// @note untuk menampilkan director pada debit note
	/// @attention Select semua data field dari table
	public function Tampil_director_filter()
	{
		return $this->db->get('tb_director')->result();
	}
